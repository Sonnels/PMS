...
class ReportForDay implements FromView, WithColumnFormatting, ShouldAutoSize
{
    /**
    ...
    */
    ...
    public function view(): View{
        ...
        // $this->f1 = $this->f1 == '' ? date('Y-m-d') : $this->f1;

        $query =    $this->f1;
        $query2 =  $this->f2 == 'TODO' ? '' : $this->f2;
        ...
        $consumo = DB::table('consumo as c')->join('reserva as r', 'c.IdReserva', 'r.IdReserva')->join('producto as p', 'c.IdProducto', 'p.IdProducto')
        ...
        foreach ($pagos as $pa) {
            $registro[] = array('hora'=> date('H:i:s', strtotime($pa->fecPag)), "tipo" => 'ALQUILER',
             'motivo'=>'PAGOS', 'entidad'=> 'Hab. N°: '. $pa->Num_Hab, "metodoPago"=> $pa->metPag, 'monto'=> $pa->monPag);
        }

        foreach ($consumo as $co) {
            $registro[] = array('hora'=>explode(' ', $co->FechaPago)[1], "tipo" => $co->Denominacion == 'Servicio' ? 'SERVICIO' : 'CONSUMO',
             'motivo'=> '',
             'entidad'=> 'Hab. N°: '. $co->Num_Hab, "metodoPago"=> $co->metodoPago, 'monto'=> $co->Total);
        }

        foreach ($venta as $ve) {
            $registro[] = array('hora'=> $ve->horaVenta, "tipo" => 'VENTA' ,
             'motivo'=> '',
        ...
        return view('exports.reportforday', ["registro"=>$registro, "responsable"=>$responsable, "searchText" =>$this->f1]);
    }
}
