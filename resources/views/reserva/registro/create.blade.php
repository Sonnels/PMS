...
@section('Contenido')
    <div class="card mt-2">
        <div class="card-header bg-secondary pb-2 pt-2">
            ...
            <span id="Num_Hab-error" class="text-danger"></span>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col-md-4">
                    <span class="text-gray-dark">PRECIO: </span><span>
                        {{ $Habitacion->precioHora }} | {{ $Habitacion->precioNoche }} | {{ $Habitacion->precioMes }}</span>
                    {{-- S/ {{ $Habitacion->Precio }} --}}
    ...
    <form id="form1" method="POST" action="{{ route('registro.store') }}">
        @csrf
        <div class="row">
            <div class="col-md-6">
            ...
            @php($servicio = ['3 HORAS', 'TODA LA NOCHE', 'POR MES'])
                            ...
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text bg-white text-muted" style="display: grid;">
                                        <div class="text-left font-medium">{{ $datos_empresa->simboloMoneda }}</div>
                                    </span>
                                </div>
                                <input type="text" id="Descuento" name="Descuento"
                                    value="{{ !old('Descuento') ? 0 : old('Descuento') }}"
                    ...
                    <div class="col-md-2">
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" id="Adelanto" name="Adelanto" value="{{ old('Adelanto', 0) }}"
        ...
        <div class="row">
            <div class="col-lg-12 col-sm-6 col-md-6 col-xs-6 text-center">
                <div class="form-group">
                    ...
                    <button class="btn btn-primary agregar" id="enviarRegistro" type="submit">Registrar</button>
                </div>
            </div>
        </div>
    </form>
    ...
    @push('scripts')
        ...
        <script>
            ...
            document.querySelector('#form1').addEventListener('submit', function(e) {
                ...
                let fechaEntrada = new Date().toLocaleTimeString() + ' ' + document.getElementById('freserva').value;
                let fechaSalida = document.getElementById('horaSalida').value + ':00 ' + document.getElementById(
                    'fsalida').value;
                ...
                Swal.fire({
                    ...
                    html: `<div class="table-responsive">
                            ...
                            <tr><td align="left"><b>Cliente:</b></td><td align="left">${cliente[1]}</td></tr>
                            <tr><td align="left"><b>Tarifa:</b></td><td align="left">${servicio}</td></tr>
                            <tr><td align="left"><b>Fecha Entrada:</b></td><td align="left">${fechaEntrada}</td></tr>
                            <tr><td align="left"><b>Fecha Salida:</b></td><td align="left">${fechaSalida}</td></tr>
                            <tr><td align="left"><b>Descuento:</b></td><td align="right">${simboloMoneda} ${descuento}</td></tr>
                            <tr class="text-success"><td align="left"><b>Pago Recibido:</b></td><td align="right">${simboloMoneda} ${adelanto}</td></tr>
                            <tr class="text-danger"><td align="left"><b>${texto}</b></td><td align="right">${simboloMoneda} ${pagar}</td></tr>
                            </table></div>
        ...
        <script>
            ...
            function FechaValida(fecha_Inicial, fecha_Final) {
                ...
                if (fecha_Inicial >= fecha_Final && (serv != '3 HORAS' && serv != '2 HORAS' && serv != '8 HORAS')) {
            ...
            const descuento_input = document.querySelector('#Descuento');
            descuento_input.addEventListener("keyup", function(event) {
                calcular_costos();
            }, false);

            const adelanto_input = document.querySelector('#Adelanto');
            adelanto_input.addEventListener("keyup", function(event) {
                calcular_costos();
            }, false);

            function calcular_costos() {
                let p_recib = document.getElementById("Adelanto").value === '' ? 0 : document.getElementById("Adelanto").value;
                let descue = document.getElementById("Descuento").value === '' ? 0 : document.getElementById("Descuento").value;
                let costoHab = document.getElementById("costo").value === '' ? 0 : document.getElementById("costo").value;
                let asignado = parseFloat(costoHab) - parseFloat(descue) - parseFloat(p_recib);
                if (asignado >= 0) {
            ...
            $('#card-familiar').CardWidget('toggle');
            // Para los huéspedes adicionales
        </script>
    @endpush
    <script>
    ...
    <script>
        ...
        cantida_m.addEventListener("keyup", function(event) {
            if (document.getElementById("cantida_m").value > 0) {
                ...
                if (document.getElementById("servicio").value == 'POR MES') {
                    document.getElementById("fsalida").value = f_actual.addMonth(parseInt(document.getElementById(
                        "cantida_m").value));
                } else if (document.getElementById("servicio").value == '3 HORAS') {
                    var fecha = new Date();
                    const cantidad_m = parseInt(document.getElementById("cantida_m").value);
                    fecha.setHours(fecha.getHours() + (3 * cantidad_m));
                    document.getElementById("fsalida").value = moment(fecha).format("YYYY-MM-DD");
